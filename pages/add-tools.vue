        ...
        <div v-if="$auth.loggedIn">
            ...
            <div class="content">
                ...
                <section class='padding'>
                    ...
                    <p v-if="isOkayToDelete=== false" class='message'>{{deleteMessage.toUpperCase()}}</p>
                    <p v-if="isOkayToDelete" class='message'>{{deleteMessage.toUpperCase()}}</p>
                    <div class='wrapperTools'>
                        <form class="formPart">
                            <div class='column'>
                                <label for="ref">Référence:</label>
      ...
      'send'(){
            ...
            if(this.tool.reference ==='' || this.tool.designation ===''){
                ...
                this.$axios.post('/api/tool/', {
                    ...
                    alert(`${this.tool.designation} ajouté! Vous pouvez l'utiliser dans vos futures listes désormais`)})
        ...
        'deleteTool'(){
            ...
            if (this.toolDeleted.reference !== '') {
                ...
                if(ref.length>0){
                    this.$axios.delete(`/api/tool/${ref[0].id}` ).then(() => {alert(`Outil supprimé`)}).then(() => {
        ...
        'checkDelete' () {
            if (this.toolDeleted.reference !== '') {
                ...
                post.reference.toLowerCase().includes(this.toolDeleted.reference.toLowerCase()))
                if(ref.length === 0){
                    this.isOkayToDelete = false
                    this.deleteMessage = `Cette référence n'existe pas`
                }
                else {
                this.isOkayToDelete = true
                this.deleteMessage = `${ref[0].designation} va être supprimé`
                }
            }
        },
        'checkData' () {
