  ...
  <div>
    <div class='length'>
      <div class='wrapper-list'> <h3>Ma Liste</h3>
        <p v-for="element in finalList" id="flex-list" :key="element.content.id" > ITEM: {{element.content.designation}} // {{element.content.reference}}- QUANTITÉ: {{element.quantity}}</p></div>
          <div class='wrapper-flex'>
            <input v-model="searchTool" type="text" class="input-search" placeholder="Rechercher du matériel" required/>
            <div class='pm-flex'>
              <input v-model="quantity" class='qt-list' type="number" min="1" max="20" placeholder="0" required>
            ...
            <section v-for="result in filteredList" id="flex-wrapper" :key="result.id"  :class="selectedTool.id === result.id ? 'active' : ''" @click="handleSelect(result)">
...
export default{
  ...
  computed: {
    'filteredList' () {
      ...
      return this.results.filter((post) => post.designation.toLowerCase().includes(this.searchTool.toLowerCase())|| post.reference.toLowerCase().includes(this.searchTool.toLowerCase()) )
    }
  },
  created(){
